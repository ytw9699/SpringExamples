...
<div class="wrapper">	
	<table class="boardView">
		...
		<tr>																
			<td colspan="4" align="left"><span class="date">첨부파일1:&nbsp;
			<a href="download?fileName=${board.fileName}" target="_blank">${board.fileName}</a></span>
			<input type="button" value="${board.fileName}" onclick="moveAction(5)" />
			</td>
			<%-- <a href="<%=request.getContextPath()%>/files/${board.fileName}" target="_blank">${board.fileName}</a></span></td> --%>
		</tr>
		</c:if>	
		<tr>
	...
	<table class="commentView">
		...
		<tr>
			...
			<td>
				<form action="AlarmWrite.do" method="post">
					<input type="hidden" name="id" value="${board.writer}" />
					<input type="hidden" name="productnumber" value="${board.idx}" />
					<input type="hidden" name="kind" value="0" />
					<input type="submit" value="알람확인" class="commentBt" />
				</form>
			</td>
		</tr>
	</table>
