...
public class NewArticleController {
	...
	//dispatcher-servlet.xml객체주입 설정에서 해줬기때문에 @Autowired빼도 사실 상관없음
	...
	}//NewArticleController 컨트롤러 객체와 ArticleService 객체가 스프링 컨테이너에 올라가게될거
	//내가작업하는 이 컨트롤러에서 아티클서비스를 가져다쓸거임 가져다쓸때는 new로 객체생성하면 되는데
	//객체선언만 해두고 객체를 받아서 쓰는거,객체를 미리 생성해두고 객체끼리 관계설정하면서 작업하는거 ,객체생성로직이빠짐
	//dispatcher-servlet.xml안에서 설정 or @Autowired가 해줌
}