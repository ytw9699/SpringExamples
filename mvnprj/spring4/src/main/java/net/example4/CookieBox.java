...
public class CookieBox {
    ...
    public CookieBox(HttpServletRequest request) {//생성자하나//리퀘스트 객체받아서 
        ...
        if (cookies != null) {//생성된 배열이 널이 아니면
            ...
            for (int i = 0 ; i < cookies.length ; i++) {
                ...
                //간편하게 내가 키값만 줘서 꺼내쓸수있기 때문에 맵을 쓴거다..!이게 핵심이야!
    ...
    public String getValue(String name) throws IOException {//이름 받아서 값을 리턴
        ...
        Cookie cookie = cookieMap.get(name);//일단 쿠키객체 얻어내고
    ...
    //밑에 createcookie 메소드들은 객체생성 필요없이 쿠키를 만들어내는것
    //위와 아래는 다른얘기
    
    public static Cookie createCookie(String name, String value) throws IOException {//메소드 오버로딩1
    	return new Cookie(name, URLEncoder.encode(value, "utf-8"));//쿠키 객체생성
    	 //쿠키의 이름과 값만 받아서 쿠키 객체 리턴
        //static이 붙어서 객체생성 필요없이 쿠키 객체생성
    }
    
    public static Cookie createCookie(String name, String value, String path, int maxAge) throws IOException {//메소드 오버로딩2 
        Cookie cookie = new Cookie(name, URLEncoder.encode(value, "utf-8"));
        cookie.setPath(path);//패스 설정
        cookie.setMaxAge(maxAge);//유효 시간 설정
        return cookie;//그리고 쿠키 리턴
    }
    public static Cookie createCookie(String name, String value, String domain, String path, int maxAge) throws IOException {//메소드 오버로딩3
        Cookie cookie = new Cookie(name, URLEncoder.encode(value, "utf-8"));
