...
//여기선 폼으로부터 데이터가 넘어온거고 그거를 memberinfo 자바빈에 설정했는데 ,그 자바빈 가주고 유효성 검증하는것 
//전송되지않은 값이있으면 오류를 내야해서
...
//BindingResult error = (BindingResult)errors;//이거는 형변환 안하고 써도됨..다형성개념
if (memberInfo.getId() == null || memberInfo.getId().trim().isEmpty()) {//trim앞뒤공백없애고 isEmpty데이터가없거나
	...
	//jsp와 연동해서 오류메시지가 출력되도록//BindingResult(Errors)객체에다가 에러관련된 메시지를 설정한다
}
