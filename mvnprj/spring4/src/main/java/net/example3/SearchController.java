...
@Controller//http://localhost:8080/spring4/example3/internal.do?query=p&p=2
   public class SearchController {
	...
	@RequestParam(value = "p", defaultValue = "1") int pageNumber) {//스트링객체로 날라와도 바로 int타입에 저장가능
		...
		ModelAndView mav = new ModelAndView("example3/internal");//생성자에다가 이렇게 넣어줌..mav.setViewName대신
		//이렇게 jsp로 포워딩
	...
	@RequestMapping("/example3/external.do")//required는 반드시요구된다는 건데 거기에 false를 줘서 반드시 날라오지않아도 된다
	public ModelAndView searchExternal(@RequestParam(value="query", required=false) String query,
		...
		System.out.println("query=" + query + ",pageNumber=" + pageNumber);
		ModelAndView mav = new ModelAndView("example3/external");
		mav.addObject("query",query);
		mav.addObject("p",pageNumber);
		return mav;
}
